...
trait NodeScala {
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    ...
    Future.run(){ct =>
      InnerStart(ct, listener, handler)
      .continue(_ => listenerSubscription.unsubscribe())
    }
  }

  private def InnerStart(ct: CancellationToken, listener: Listener, handler: Request => Response): Future[Unit] = async{
    while (ct.nonCancelled){
      val req = await {listener.nextRequest()}
      val resp = handler(req._1)
      respond(req._2, ct, resp)
    }
  }
}
